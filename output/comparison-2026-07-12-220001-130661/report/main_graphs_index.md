...
- `throughput_vs_load.png`: `successful_rps` from `repetition_values.csv` (per-run dots with a median line; n=5 individual values per load level).
...
- `p95_latency_vs_load.png`: `p95_latency_ms` from `repetition_values.csv` (HDR-derived; per-run dots with a median line; no CI).
- `p99_latency_vs_load.png`: `p99_latency_ms` from `repetition_values.csv` (HDR-derived; per-run dots with a median line; no CI).
