...
| Metric at 64 offered RPS | HikariCP | Open J Proxy | PgBouncer |
|---|---|---|---|
...
flowchart LR
    ...
    subgraph Open J Proxy
        direction LR
        A3[App Replica × 16] -->|virtual connections| OJ[Open J Proxy Server × 3] -->|≤53 connections| DB3[(PostgreSQL)]
    end
```

| | HikariCP | Open J Proxy | PgBouncer |
|---|---|---|---|
...
| Proxy tier | None | 3 Open J Proxy nodes | 3 PgBouncer nodes + 1 HAProxy node |

HikariCP is the direct baseline: each of the sixteen replicas holds its own local connection pool, so the total number of open PostgreSQL connections grows with the replica count — reaching 312 at higher load levels. PgBouncer interposes a pooler layer fronted by HAProxy, capping backend connections to around 56. Open J Proxy removes client-side pooling from the application entirely; virtual JDBC connections are multiplexed through a three-node Open J Proxy server tier, again limiting real database connections to around 53.
...
```mermaid
xychart-beta
    title "Successful throughput vs load (line order: HikariCP, Open J Proxy, PgBouncer)"
...
| Offered load | HikariCP | Open J Proxy | PgBouncer |
|---|---|---|---|
...
```mermaid
xychart-beta
    title "Error rate vs load (line order: HikariCP, Open J Proxy, PgBouncer)"
    x-axis ["16 RPS", "32 RPS", "48 RPS", "64 RPS"]
...
One notable result at the lowest load level is that Open J Proxy recorded a 1.4% error rate while HikariCP and PgBouncer were effectively error-free. Those Open J Proxy errors were `SQLTransientConnectionException` rejections — Open J Proxy's internal queue-limit firing when a virtual connection could not acquire an operation slot within the configured timeout. They appeared across all five repetitions at this load level, which suggests they reflect a consistent characteristic of the Open J Proxy queue configuration rather than an anomalous run. HikariCP and PgBouncer did not encounter this kind of fast rejection at low load because they accepted all requests into their queues and served them within the timeout window.

At higher loads, HikariCP errors reach 39% at 32 RPS and 66% at 64 RPS. PgBouncer reaches 59%. Open J Proxy errors grow more slowly, reaching 36% at the highest tested load level.
...
Open J Proxy behaves differently.

```mermaid
xychart-beta
    title "Mean failed-request latency vs load (line order: HikariCP, Open J Proxy, PgBouncer)"
    x-axis ["16 RPS", "32 RPS", "48 RPS", "64 RPS"]
...
At 64 RPS, Open J Proxy's mean failed-request latency is roughly **3.2 seconds**. At 16 RPS it averages about 13 seconds — those are the same queue-limit rejections described above — and the latency decreases as load rises, indicating that Open J Proxy's backpressure mechanism rejects congested requests more quickly when the system is under greater pressure. For HikariCP and PgBouncer the failure latency stays flat at the timeout ceiling regardless of load.

## The Latency of What Succeeds

The flip side of Open J Proxy's backpressure behaviour is that requests which do succeed are served with considerably lower tail latency. The p95 successful latency shows this most clearly.

```mermaid
xychart-beta
    title "p95 successful latency vs load in ms (line order: HikariCP, Open J Proxy, PgBouncer)"
    x-axis ["16 RPS", "32 RPS", "48 RPS", "64 RPS"]
...
| Offered load | HikariCP p95 | Open J Proxy p95 | PgBouncer p95 |
|---|---|---|---|
...
At 32 RPS, Open J Proxy's p95 successful latency is **49 milliseconds**. HikariCP's is 121,966 milliseconds and PgBouncer's is 45,210 milliseconds. The HikariCP figure is not erroneous: because latency is measured end to end from when the load generator schedules each request, a request that queues for a worker thread, then waits for a pool connection, can accumulate substantially more than one connection-timeout period before completing. Open J Proxy avoids this by rejecting requests it cannot serve quickly, so requests that make it through are not delayed by a long queue ahead of them. The gap narrows at higher loads but Open J Proxy's successful p95 remains substantially lower across all tested load points.
...
```mermaid
xychart-beta
    title "PostgreSQL RSS vs load in MiB (line order: HikariCP, Open J Proxy, PgBouncer)"
    x-axis ["16 RPS", "32 RPS", "48 RPS", "64 RPS"]
...
Under Open J Proxy, PostgreSQL RSS stays between **17 and 25 GiB** across all load levels. Under HikariCP it grows from about 20 GiB at low load to over 63 GiB at full load, tracking the backend connection count: HikariCP eventually opens 312 connections, and each carries process memory and working buffers. Open J Proxy holds backend connections to around 53 and PostgreSQL memory grows accordingly.

The PgBouncer result is harder to explain. PgBouncer also constrains backend connections to around 56, similar to Open J Proxy, yet PostgreSQL RSS starts at approximately 99 GiB at the lowest load level and climbs to 110 GiB — substantially more than even HikariCP at peak. These measurements do not establish a cause. The result may reflect workload behaviour, PostgreSQL memory accounting, retained allocations, the way RSS was aggregated across PostgreSQL processes (which can cause shared memory pages to be counted more than once), or something specific to the PgBouncer session model. Further investigation using proportional set size, cgroup memory, and PostgreSQL memory instrumentation is needed before attributing a cause.

## How Backend Connections Are Used

With Open J Proxy and PgBouncer both maintaining a similar number of real PostgreSQL connections, it is worth examining how much successful work each backend connection produces.

```mermaid
xychart-beta
    title "Successful RPS per DB connection (line order: HikariCP, Open J Proxy, PgBouncer)"
    x-axis ["16 RPS", "32 RPS", "48 RPS", "64 RPS"]
...
At 64 RPS, Open J Proxy delivers **0.75 successful requests per second per backend connection**, compared to 0.47 for PgBouncer and 0.07 for HikariCP. Open J Proxy's higher throughput per connection is consistent with its lower error rate and faster failure reporting: fewer backend connections are occupied serving requests that will ultimately be rejected, so a greater proportion of connection time goes toward requests that succeed.

## Open J Proxy's Own Resource Cost

Open J Proxy runs a three-node JVM proxy tier that carries resource costs not present in the other two topologies.

The Open J Proxy proxy tier uses around **1,023 to 1,079 MiB** of RSS across its three nodes combined. PgBouncer's equivalent tier — three PgBouncer nodes plus HAProxy — uses about **56 MiB** total, making the Open J Proxy proxy tier roughly 18 times heavier in memory. That difference is a real cost that should be factored into any deployment decision.

What is notable is that the Open J Proxy JVM overhead is stable across load levels. Heap used stays between **88 and 90 MiB** cluster-wide regardless of load, indicating the JVM is not under memory pressure at peak. The committed heap — memory the JVM has reserved for reuse — sits around 150–161 MiB. The proxy tier memory is an upfront cost that does not scale meaningfully with offered load.

PostgreSQL CPU under Open J Proxy is higher than under HikariCP in absolute terms: around 483% CPU at 64 RPS compared to 262% for HikariCP (where 100% represents one fully utilised core on the multi-core database node). Open J Proxy is simultaneously delivering 85% more successful work at that load level, so CPU per successful request is more favourable than the raw figure suggests. PgBouncer's PostgreSQL CPU is the highest of all three — 933% at 64 RPS — while also delivering lower successful throughput than Open J Proxy, which makes it the most CPU-intensive option per unit of useful work completed.

## System-Wide Resource Totals

Viewing the proxy tier in isolation understates Open J Proxy's advantage. The table below sums PostgreSQL RSS and proxy-tier RSS to give the full database-plus-proxy infrastructure footprint at each load level.

| Offered load | HikariCP total RSS | Open J Proxy total RSS | PgBouncer total RSS |
|---|---|---|---|
...
The configurations were not designed to provide each technology with an identical backend connection count. They modelled intended production deployment patterns: independently sized local pools for HikariCP, and centrally bounded backend capacity for Open J Proxy and PgBouncer. The difference in database pressure — 312 connections for HikariCP versus roughly 53–56 for the others — is part of what each architecture produces, not an independently controlled variable. Additional experiments with equalized connection budgets would help separate architectural effects from configuration effects.
...
The potential benefit of a control-plane model goes beyond throughput numbers. Even in extreme overload scenarios, a centralized capacity controller can keep doing consistent, useful work — accepting and completing requests at a predictable rate — while failing fast on the excess. Requests that cannot be served are rejected in seconds rather than held for 30 seconds before timing out, which is meaningful for clients and for the database itself. Without this kind of backpressure, independent local pools can collectively push far more connections and in-flight work into the database than it can sustain, triggering the kind of cascading degradation described as [The Database Death Spiral](https://rogeriorobetti.medium.com/the-database-death-spiral-70aaab5dc993). Whether a control-plane approach delivers the same properties on other workloads, database sizes, or traffic patterns is not established by this benchmark — but it is a model worth considering when predictable failure behaviour under load matters.
